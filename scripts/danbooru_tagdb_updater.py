...
def load_config() -> dict:
    try:
        ...
        return {"auto_update": True, "interval_days": DEFAULT_INTERVAL_DAYS, "tagcomplete_dir": ""}


def save_config(auto_update: bool, interval_days: int, tagcomplete_dir: str) -> None:
    data = {
...
def run_update(path_text: str, force: bool, interval_days: int) -> tuple[str, str]:
    result = update_tagdb(
...
def build_ui():
    ...
    with gr.Blocks() as block:
        ...
        result = gr.Textbox(label="結果", lines=4, interactive=False)
        info = gr.Textbox(label="状態", value=current_info(), lines=6, interactive=False)

        def save_fn(a, d, p):
            save_config(a, int(d), p)
            return "設定を保存しました。", current_info()

        save.click(save_fn, inputs=[auto, days, path], outputs=[result, info])
        update.click(run_update, inputs=[path, force, days], outputs=[result, info])
